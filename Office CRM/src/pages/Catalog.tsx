...
import { Search, Filter, RefreshCw, Package, Copy, X, Tag, Images, BookOpen, ChevronLeft, ChevronRight } from 'lucide-react';
...
const prefetchFolderImages = async (folderId: number) => {
  ...
  try {
    const res = await fetch(`http://localhost:8000/api/v1/catalog/folders/${folderId}/images`);
    ...
    [...parsed.aiPreviews.slice(0, 3).map(p => p.image), ...firstViews.map(v => v.image)].forEach(img => {
      ...
      if (id) { const l = new Image(); l.src = `http://localhost:8000/api/v1/catalog/image-proxy?id=${id}&sz=800`; }
    });
  } catch {}
...
export function Catalog() {
  ...
  const fetchFolders = async (reset = false) => {
    ...
    try {
      ...
      let url = `http://localhost:8000/api/v1/catalog/folders/?skip=${skip}&limit=24&sort_by=alphabetical`;
  ...
  const handleOpenFolder = async (folder: CatalogFolder) => {
    ...
    if (cached) {
      ...
      try {
        const res = await fetch(`http://localhost:8000/api/v1/catalog/folders/${folder.id}/images`);
        const data: CatalogImage[] = await res.json();
  ...
  const getImageUrl = (url: string, width: number = 800) => {
    ...
    if (id) return `http://localhost:8000/api/v1/catalog/image-proxy?id=${id}&sz=${width}`;
    return url;
  };
          ...
          >
            <div className="aspect-[4/5] bg-slate-100 relative overflow-hidden shrink-0">
              {folder.thumbnail_url ? (
                <img
                  src={getImageUrl(folder.thumbnail_url, 400)}
                    ...
                    {loadingImages ? (
                      ...
                      <>
                        <img
                          ...
                          src={getImageUrl(currentProductImage.image_url, 800)}
                          alt=""
