...
import {
  ...
  Activity, Award, Target, Zap, X, ShoppingBag,
} from 'lucide-react';
...
export function CustomerLists() {
  ...
  const copyImageToClipboard = async (productId: string, imageUrl: string) => {
    setCopyingImgId(productId);
    try {
        ...
        <AnimatePresence mode="wait">
          {selectedCustomer ? (
            <SlideInRight key={selectedCustomer.id} className="min-h-full p-4 md:p-8 max-w-6xl mx-auto space-y-8">
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {customerSalesOrders.map((o) => (
                          <tr key={o.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="py-2 text-slate-600">{o.productCode} ({o.color})</td>
              ...
              <FadeIn delay={0.4} className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
                {/* Background glow effects */}
                <div className="absolute top-0 right-0 w-32 h-32 rounded-full bg-brand-primary/20 blur-2xl -mr-8 -mt-8" />
                <div className="absolute bottom-0 left-0 w-24 h-24 rounded-full bg-emerald-500/15 blur-xl -ml-4 -mb-4" />
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-1 font-medium">Copy mock product images and structured text descriptions to paste directly in WhatsApp chat.</p>
                  </div>
                  <Button
                    onClick={() => setShowWhatsApp(true)}
                    className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl flex items-center gap-2 shrink-0 self-start md:self-auto"
                  >
                    <MessageCircle className="h-4 w-4" /> Open WhatsApp Panel
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  ...
                  {Array.from(new Set(recommendations.flatMap(r => r.products))).slice(0, 10).map((p) => {
                    ...
                    return (
                      <div key={p.id} className="bg-white/5 border border-white/10 rounded-2xl p-3 flex flex-col justify-between hover:bg-white/10 transition-colors group">
                        <div className="relative aspect-[3/4] rounded-xl overflow-hidden border border-white/10 mb-3 bg-black">
                          <img 
                            src={p.imageUrl || getLehengaFallback(p.id)} 
                            onError={(e) => { e.currentTarget.src = getLehengaFallback(p.id); }}
                            alt={p.name} 
                          ...
                          />
                          <div className="absolute bottom-2 left-2 bg-black/60 backdrop-blur-sm px-2 py-0.5 rounded-lg text-[9px] font-bold">
                            {p.designCode}
                          </div>
                        </div>

                        <div>
                          <h4 className="text-xs font-bold text-slate-200 line-clamp-1 mb-1">{p.name}</h4>
                          <p className="text-[10px] text-slate-400 font-bold mb-3">₹{p.priceBucket} | {p.color}</p>
                          
                          <div className="grid grid-cols-2 gap-2 text-slate-900">
                            <Button
                              size="sm"
                              onClick={() => copyImageToClipboard(p.id, p.imageUrl || getLehengaFallback(p.id))}
                              disabled={isImgCopying}
                              className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800 text-white text-[10px] font-bold rounded-xl h-8 px-1"
                            >
                              {isImgCopying ? '...' : 'Copy Image'}
                            </Button>
                            <Button
                              size="sm"
                              onClick={() => copyTextToClipboard(p.id, descText)}
                              className={`${
                                isTextCopied ? 'bg-emerald-600 text-white hover:bg-emerald-600' : 'bg-slate-100 text-slate-900 hover:bg-slate-200'
                              } text-[10px] font-bold rounded-xl h-8 px-1`}
                            >
